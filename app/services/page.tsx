...
import { formatPrice, serviceHref } from "../../lib/sedifex-public";
    ...
    <>
      <section className="hero-grid">
        <div className="section py-20">
          ...
          <h1 className="mt-6 max-w-4xl text-5xl font-black tracking-tight text-slate-950 md:text-7xl">
            Germany Nursing Pathway Support
          </h1>
          <p className="mt-6 max-w-3xl text-xl leading-9 text-slate-700">
            Book guidance for Nursing Ausbildung, FSJ, BFD, Au-Pair, Recognition,
            Document Review and Student Visa preparation.
          </p>
      ...
      <section className="section">
        ...
        <p className="section-subtitle">
          Services are loaded from Sedifex service products, including known public listing ids when the standard product list returns empty.
        </p>

        <div className="mt-10 grid gap-6 md:grid-cols-3">
          {services.map((service) => (
            <article key={service.id} className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-xl">
              <div className="relative aspect-[16/10] bg-gradient-to-br from-emerald-100 via-white to-amber-100">
                {service.imageUrl ? (
                  <Image src={service.imageUrl} alt={service.imageAlt || service.name} fill sizes="(min-width: 768px) 33vw, 100vw" className="object-cover" />
                ) : (
              ...
              <div className="p-6">
                ...
                <div className="mt-6 flex items-center justify-between gap-3">
                  <span className="font-black text-slate-950">From {formatPrice(service.price)}</span>
                  <Link href={serviceHref(service)} className="rounded-full bg-slate-950 px-4 py-2 text-sm font-bold text-white hover:bg-emerald-700">View</Link>
                </div>
              </div>
            </article>
          ))}
